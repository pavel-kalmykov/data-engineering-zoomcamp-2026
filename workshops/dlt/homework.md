...
```sql
SELECT MIN(trip_pickup_date_time AT TIME ZONE 'UTC')::DATE AS start_date,
       MAX(trip_pickup_date_time AT TIME ZONE 'UTC')::DATE AS end_date
...
**dlt MCP server (`execute_sql_query`):**

```json
{"result":[["2009-06-01","2009-06-30"]]}
```

The timestamps are stored with timezone info, so casting to UTC first avoids a local-offset shift. The dataset is June 2009.
...
```sql
SELECT ROUND(SUM(CASE WHEN payment_type = 'Credit' THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) AS credit_card_pct
FROM taxi_data.rides;
```
...
`payment_type` comes as a string with values `'Credit'`, `'CASH'`, `'Cash'`, `'No Charge'`, and `'Dispute'`.
...
The tip column is `tip_amt` (not `tip_amount`).
