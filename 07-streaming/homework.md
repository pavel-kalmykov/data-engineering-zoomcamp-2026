...
**Answer: `10 seconds`** (took ~5 seconds)
...
Job: `src/job/job_q4_tumbling_pu.py`

```sql
SELECT pulocationid, num_trips FROM pu_trips_5min ORDER BY num_trips DESC LIMIT 3;
```
...
Job: `src/job/job_q5_session_pu.py` (SESSION TVF con `PARTITION BY PULocationID`)

```sql
SELECT pulocationid, num_trips FROM pu_session_trips ORDER BY num_trips DESC LIMIT 3;
```
...
Job: `src/job/job_q6_hourly_tips.py`
